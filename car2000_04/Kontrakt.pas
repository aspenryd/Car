...
{**********************************************************************}
{}
{ $Log:  14856: Kontrakt.pas
{
{   Rev 1.5    2008-06-27 13:33:44  Henry
{ Lagt till hantering av stationer
{
}
{   Rev 1.4    2006-02-18 13:33:44  pb64
}
{
{   Rev 1.3    2005-02-14 16:13:02  pb64
{ Fixat visning av bilder och referensfältets längd
}
{
{   Rev 1.2    2005-02-07 11:22:28  pb64
}
{
{   Rev 1.1    2004-10-26 15:46:54  pb64
}
{
...
uses
  ...
  Grids, comp2000, ActnList, tmpData, eqprn, EQFormatEdit,
  EQDateEdit, DBGrids, ADODB, uCarLegal, ShellAPI;
...
type
  ...
  TFrmKontrakt = class(TForm)
    ...
    BitBtn9: TBitBtn;
    DBGrid1: TDBGrid;
    dsBookingSearch: TDataSource;
    Button1: TButton;
    lblCount: TLabel;
    Button2: TButton;
    checkMailBooking: TCheckBox;
    edMailAdress: TEdit;
    Panel4: TPanel;
    BitBtn10: TBitBtn;
    ComboBox1: TComboBox;
  ...
  private
    ...
    procedure SearchObject(obtyp: string; StationIndex : integer = 0);
  ...
  public
    ...
    function GetKM(Regno: string): Integer;
...
const
  SQLobj = 'Select type, REG_NO, Priceclass, MODEL, Accesories , Color , KM_N , ([n_service]-[km_n]) AS Service, ObjNum, VStat, Station.Name as Station, ClearForRent as Uthyrningsbar from objects left outer join Station on (Objects.Station = Station.StationId)'; //! Benny Lagt tyill Vstat
...
//!  stdDays:Integer;
  ...
  stdDays: Integer;
  QBooking: TAdoQuery;
  ContractHasAnInvoice: Boolean;
  frmCarLegalHolder: TfrmCarlegal;
  CheckCustomerString: string;
...
begin
  ...
  MomsCountConst := StrToInt(GetValueFromInit('CAR', '!MM', 'MomsCountConst', '1'));

  checkDelfakt.Checked := False;
  checkMailBooking.Checked := False;
  edMailAdress.Text := '';
  ...
  case view of
    booking:
      begin
        ...
        SetupQBooking;
        QBooking.Open;
        lblCount.Caption := 'Poster : ' + IntToStr(QBooking.RecordCount);
        Panel2.Visible := True;
...
begin
  ...
  cbPristyp.ItemIndex := 0;
//!  ClearObject;
  if (QBooking.Active) and (CurrentView = booking) then
    SetupQBooking;
end;
...
var
  pWhere: Integer;
begin
  if not assigned(frmCarLegalHolder) then
  begin
    frmCarLegalHolder := TfrmCarlegal.Create(nil);
  end;
  internkund := False;
  screen.cursor := crHourglass;
  pWhere := Pos('WHERE', uppercase(SQLBokningHelp2));
  if CurrentView = booking then
  begin
    QBooking := CreateDS(copy(SQLBokningHelp2, 1, pWhere - 1));
    QBooking.SQL.Add('');
    QBooking.SQL.Add('');
    dsBookingSearch.DataSet := QBooking;
  end;
  ClearAll;
  CreateObj(1);
  cbObjtyp.Itemindex := 0;
...
begin
  ...
  begin
    ...
    SearchObject(cbObjTyp.text, frmMain.GetStationIndex);
  end;
...
procedure TFrmKontrakt.SearchObject(obtyp: string; StationIndex : integer = 0);
var
  Obnr: string;
  ObjStr: string;
  PB_FIX: Boolean;
begin
  ...
  begin
    if pos ('WHERE', frmSearch.SSQLString) > 0 then
      frmSearch.SSQLString := frmSearch.SSQLString + ' AND Station = ' + inttostr(Stations[StationIndex-1].ID)
    else
      frmSearch.SSQLString := frmSearch.SSQLString + ' WHERE Station = ' + inttostr(Stations[StationIndex-1].ID);
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (strtoint(edtKM_ut.Text) > (frmgreg.ObjectsT.FieldByName('N_Service').asinteger - 500)) and (strtoint(edtKM_ut.Text) <> 0) then
          begin
            FrmDg_ObjService := tFrmDg_ObjService.Create(nil);
            try
            ...
            FrmDg_ObjService.Free;
            FrmDg_ObjService := nil;
          end;
        end;
      end;
  ...
  if frmSearch.GetSearchResult then
    ...
    begin
      ...
      begin
        ...
        edMailAdress.Hint := Fieldbyname('MAILADRESS').asstring;

        if ((checkMailBooking.Checked) or (Fieldbyname('MAILADRESS').asstring <> '')) and (CurrentView = booking) then
        begin
...
function TFrmKontrakt.GetKM(Regno: string): Integer;
var
  ds: tAdoQuery;
begin
  Result := 0;
  ...
  try
    ds.Active := True;
    Result := ds.fieldbyname('KM_N').AsInteger;
    ds.Active := False;
  except
  end;
  FreeDS(ds);
  ...
  if frmSearch.GetSearchResult then
    ...
    begin
      ...
      begin
        ...
        edtRef.MaxLength := fieldbyname('Referens').Size;
        ...
        MomsCountConst := fieldbyname('VATCOSTTYPE').AsInteger;

        checkMailBooking.Checked := fieldbyname('MAILBOOKING').AsInteger = 1;
        edMailAdress.Text := fieldbyname('MAILADRESS').AsString;
...
//!    frmSearch.GSQLString := 'Select * from [Contr_Comp] Where Contrid = '+ inttostr(ContrId);
    ...
    frmSearch.GetContrImage(ContrID, frmCarLegalHolder.Image1);
//    Image1.Picture.assign(frmSearch.GSearchCDS.FieldByName('CARLEGAL'));
    ...
    if frmSearch.GetSearchResult then
      ...
      begin
        ...
        if CurrentView = Return then
          if IsContrIdFakturerad(FieldbyName('Contrid').AsInteger) then
            ContractHasAnInvoice := True;
        next;
        while not EOF do
        begin
...
//!    PNumR:=PNumR+'-'+Copy(dmsession.SDbAnswerT.fieldbyname('KundNr').asString,7,4);
  ...
  begin
    frmSearch.GetContrImage(ContrID, frmCarLegalHolder.Image1);
    frmCarLegalHolder.ShowModal;
  end;
  if key = VK_F4 then
  begin
    if StrToBool(GetValueFromInit('CAR', '!MM', 'BC', 'False')) then
    begin
      CheckCustomerString := '"https://www.businesscheck.se/Customer/QuickSearch.aspx' +
        '?CustomerLoginName=' + GetValueFromInit('CAR', '!MM', 'BCCustomerLoginName', '') +
        '&UserLoginName=' + GetValueFromInit('CAR', '!MM', 'BCUserLoginName', '') +
        '&Password=' + GetValueFromInit('CAR', '!MM', 'BCPassword', '');
      if trim(edtHpnum.Text) <> '' then
      begin
        if copy(trim(edtHpnum.Text), 1, 2) = '55' then
        begin
          CheckCustomerString := CheckCustomerString + '&Type=Company';
          CheckCustomerString := CheckCustomerString + '&OrganizationNumber=';
          CheckCustomerString := CheckCustomerString + edtHpnum.Text;
        end
        else
        begin
          CheckCustomerString := CheckCustomerString + '&Type=Person';
          CheckCustomerString := CheckCustomerString + '&PersonNumber=';
          CheckCustomerString := CheckCustomerString + edtHpnum.Text;
        end;
      end;
      CheckCustomerString := CheckCustomerString + '"';
      Shellexecute(Handle, 'open', pchar('explorer'), pchar(CheckCustomerString), nil, sw_show);
    end;
  end;
...
begin
  ...
  try
    ...
    frmsearch.SaveContrBase(ContrID, CustId, frmMain.sign, edtRef.Text, Status, PayAlts[cbBetalning.itemindex].Code, strtoint(pnlBetVillkor.Caption),
      checkDelfakt.Checked, edDF_StartDatum.Text, edDF_SlutDatum.Text, edDF_BELOPP.Text, MomsCountConst, edDF_Fakt2Datum.Text, edDF_ACKBelopp.Text,
      checkMailBooking.Checked, edMailAdress.Text);
    ...
    begin
      ...
      begin
        ...
        if (Makeint(FedtKM_IN) = 0) and (Status = 4) then
        begin
          tmpStr := fEdtObjId;
          OLD_KM_N := FedtKM_UT;
          if Makeint(FedtKM_UT) < GetKM(FedtObjId) then
          begin
            FedtKM_UT := IntToStr(GetKM(FedtObjId));
          end;
          Dmod.ADODBLogWriter.WriteToLog('KM ' + tmpStr + ' : OLD KM_N ' + OLD_KM_N + '  NEW KM_N ' + FedtKM_UT);
        end;
...
//!Benny för att lägga ner km ställning
      begin
        ...
        with objArr[i] do
        begin
          dmod.q1.SQL.Text := 'SELECT KM_N FROM Objects WHERE  Reg_No=' + dbdel + fEdtObjId + dbdel;
          tmpStr := fEdtObjId;
        end;
        dmod.Q1.Open;
        OLD_KM_N := dmod.q1.fields[0].AsString;
        dmod.Q1.Active := False;
        Dmod.ADODBLogWriter.WriteToLog('KM ' + tmpStr + ' : OLD KM_N ' + OLD_KM_N);
        with objArr[i] do
          dmod.q1.SQL.Text := 'Update Objects Set KM_N=' + fEdtKm_In + ' where Reg_No=' + dbdel + fEdtObjId + dbdel + ' and KM_N<' + fEdtKm_In;
        Dmod.ADODBLogWriter.WriteToLog('KM ' + tmpStr + ' : ' + dmod.q1.SQL.Text);
        dmod.Q1.ExecSQL;
        dmod.Q1.Active := False;
        ...
        OLD_KM_N := dmod.q1.fields[0].AsString;
        dmod.Q1.Active := False;
        Dmod.ADODBLogWriter.WriteToLog('KM ' + tmpStr + ' : NEW KM_N ' + OLD_KM_N);

        try
          frmGReg.ObjectsT.Refresh;
        except
        end;
      end;
//!Hit
    ...
    FrmSearch.SaveContrImage(ContrID, frmCarLegalHolder.Image1);
    //Save contract costs
    for I := 1 to SGCost.RowCount - 1 do
      if sgCost.Cells[0, I] > '' then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (enum = 0) and (SumFaktFromKoref(ContrID, STAT_DELFAKTURA) > 0) and (i = 0) then
          begin
            enum := GetEnr(DelLista[i].FBetsatt);
          end;
      ...
      begin
        ...
        if (enum = 0) and (SumFaktFromKoref(ContrID, STAT_DELFAKTURA) > 0) then
        begin
          enum := GetEnr(PayAlts[cbBetalning.itemindex].Code);
        end;
        if enum > 0 then
        begin
          dmod.Q1.Active := False;
...
//!TEst Hit
    ...
    begin
      ...
      if MakeInt(edtDeposit.Text) <> 0 then
      begin
        if frmMain.DepositionsFaktura then
        begin
          Enum := DepositionFakturera(MainSub);
          if PayAlts[cbBetalning.itemindex].Code = 'F' then
            GetPrintRules('K', NumCopies, PrintType)
          else
            GetPrintRules(PayAlts[cbBetalning.itemindex].Code, NumCopies, PrintType);
          eqprn.PrintKvitto(0, 0, Enum, NumCopies, False);
        end;
      end
      else
      begin
        if SumFaktFromKoref(ContrID, STAT_DEPOSITION) <> 0 then
        begin
          if frmMain.DepositionsFaktura then
          begin
            Enum := DepositionFakturera(MainSub);
    ...
    if DontClose then
    begin
      LastDate := DTVFrom.DateTime;
      ClearAll;
      CreateObj(1);
      cbObjtyp.Itemindex := 0;
      DTVFrom.DateTime := LastDate;
      cbObjtypChange(nil);
      SetupQBooking;
//      LoadCosts;
    end
    ...
    begin
      begin
        try
          if frmmain.IsContractOpen(booking, K) then
          begin
            frmMain.frmKontraktLista[K].SetupQBooking;
          end;
        except
        end;
...
//!   dlgDelbetalare.Label4.Caption := GetKNamn(DM2.KundrT,FKontaktID);
    ...
    begin
      ...
      dlgDelbetalare.Edit3.Text := VisibleDateToStr(FSkadeDat);
  ...
  begin
    ...
    begin
      ...
      if dlgDelbetalare.Edit3.Text > '!' then
        FSkadeDat := VisibleStrToDate(dlgDelbetalare.Edit3.Text);
...
//!              mtConfirmation, [mbNo, mbYes], 0) = mrNo then
    ...
    if view in [contract] then
    begin
      if (CustId < 1) then
        raise EUserObjError.create('Du måste ange en hyresman', edtHpnum, 1);
    end;
...
var
  Q: TADOQuery;
begin
  if edtRef.MaxLength = 0 then
  begin
    Q := CreateDS('SELECT REFERENS FROM CONTR_BASE WHERE 1=2');
    try
      Q.Open;
      edtRef.MaxLength := Q.fieldbyname('Referens').Size;
      Q.Close;
    except
    end;
    FreeDS(Q);
...
begin
  ...
  begin
    try
      TEQDateEdit(Sender).Text := VisibleDateToStr(VisibleStrToDate(TEQDateEdit(Sender).Text) + Stepping);
    except
      TEQDateEdit(Sender).Text := VisibleDateToStr(Now);
    end;
    key := 0;
  end;
  if Key = VK_DOWN then
...
procedure TFrmKontrakt.DBGrid1DblClick(Sender: TObject);
begin
  DontClose := True;
  LoadContract(DBGrid1.DataSource.DataSet.FieldByName('ContrId').AsInteger);
  edtObjId.SetFocus;
end;

procedure TFrmKontrakt.BitBtn9Click(Sender: TObject);
begin
  DontClose := True;
  LoadContract(DBGrid1.DataSource.DataSet.FieldByName('ContrId').AsInteger);
  edtObjId.SetFocus;
...
procedure TFrmKontrakt.SetupQBooking;
var
  str: string;
begin
  if not assigned(QBooking) then
    Exit;
  QBooking.SQL[1] := ' WHERE (Contr_base.Status<3) ';
  if cbObjtyp.ItemIndex > 0 then
  begin
    str := GetObjTypNr(cbObjtyp.Text);
    QBooking.SQL[1] := QBooking.SQL[1] + 'AND (Contr_objt.ObTypId=''' + Str + ''') ';
  end;

  str := Copy(InternalDateToStr(DTVFrom.DateTime), 1, 10);
  QBooking.SQL[1] := QBooking.SQL[1] + 'AND (convert(varchar(10),Contr_objt.Frm_Time,120)=''' + Str + ''') ';

  if frmMain.GetStationIndex > 0 then
    QBooking.SQL[1] := QBooking.SQL[1] + 'AND (Objects.Station = ' + inttostr(Stations[frmMain.GetStationIndex-1].Id) + ') ';

  if QBooking.SQL[2] = '' then
    QBooking.SQL[2] := ' ORDER BY Contr_objt.Frm_Time, Contr_objt.To_Time desc ';
  QBooking.Open;
  lblCount.Caption := 'Poster : ' + IntToStr(QBooking.RecordCount);
end;

procedure TFrmKontrakt.DBGrid1TitleClick(Column: TColumn);
begin
  if not assigned(QBooking) then
    Exit;
  if QBooking.SQL[2] = ' ORDER BY ' + IntToStr(Column.Index + 1) then
  begin
    QBooking.SQL[2] := ' ORDER BY ' + IntToStr(Column.Index + 1) + ' desc';
  end
  else
    if QBooking.SQL[2] = ' ORDER BY ' + IntToStr(Column.Index + 1) + ' desc' then
    begin
      QBooking.SQL[2] := ' ORDER BY ' + IntToStr(Column.Index + 1);
    end
    else
    begin
      QBooking.SQL[2] := ' ORDER BY ' + IntToStr(Column.Index + 1);
...
procedure TFrmKontrakt.Button1Click(Sender: TObject);
var
  str: string;
  Q, q2: TAdoQuery;
  FoundCar: Boolean;
begin
  ...
  begin
    ...
    if dbGrid1.DataSource.DataSet.FieldByName('RegNr').AsString = '' then
    begin
      if frmMain.GetStationIndex > 0 then
        Q := CreateDS('Select type, REG_NO, Priceclass, MODEL, Accesories , Color , KM_N , ([n_service]-[km_n]) AS Service, ObjNum, VStat, Station from objects WHERE (TYPE = ''' + dbGrid1.DataSource.DataSet.FieldByName('Grupp').AsString + ''')AND (VStat <= 2) and Station = ' + dbGrid1.DataSource.DataSet.FieldByName('Station').AsString)
      else
        Q := CreateDS('Select type, REG_NO, Priceclass, MODEL, Accesories , Color , KM_N , ([n_service]-[km_n]) AS Service, ObjNum, VStat, Station from objects WHERE (TYPE = ''' + dbGrid1.DataSource.DataSet.FieldByName('Grupp').AsString + ''')AND (VStat <= 2)');
      Q.Open;
      FoundCar := False;
      while (not Q.Eof) and (not FoundCar) do
      begin
        if frmSearch.CheckObjFree(Q.FieldByName('REG_NO').AsString, DBGrid1.DataSource.DataSet.fieldByName('Från').AsDateTime, DBGrid1.DataSource.DataSet.fieldByName('Till').AsDateTime, DBGrid1.DataSource.DataSet.fieldByName('ContrId').AsInteger, False) then
        begin
          FoundCar := True;
        end;
        if not FoundCar then Q.Next;
      end;
      if FoundCar then
      begin
        dbGrid1.DataSource.DataSet.Edit;
        dbGrid1.DataSource.DataSet.FieldByName('RegNr').AsString := Q.FieldByName('REG_NO').AsString;
...
procedure TFrmKontrakt.Button2Click(Sender: TObject);

begin

  frmMain.LoadContract(DBGrid1.DataSource.DataSet.FieldByName('ContrId').AsInteger);
  dsBookingSearch.DataSet := QBooking;
  SetupQBooking;
...
var
  ...
  try
    ...
    ds := CreateDS('Select Objects.KM_N from Objects Where REG_NO = ''' + dbGrid1.DataSource.DataSet.FieldByName('RegNr').AsString + '''');
    try
      ds.Active := True;
      kmOut := ds.fieldbyname('KM_N').AsInteger;
      ds.Active := False;
    except
    end;
    ci := DBGrid1.DataSource.DataSet.FieldByName('ContrId').AsInteger;


    SetDS(ds, 'UPDATE CONTR_BASE SET STATUS=4 WHERE STATUS=2 AND CONTRID=' + IntToStr(Ci));
    ExecDS(ds);
    SetDS(ds, 'UPDATE CONTR_OBJT SET STATUS=1, KM_OUT='+IntToStr(kmOut)+' WHERE STATUS=0 AND CONTRID=' + IntToStr(Ci));
    ExecDS(ds);
    FreeDS(ds);
    SetupQBooking;
...
procedure TFrmKontrakt.BitBtn10Click(Sender: TObject);
var
  dsMail: TADOQuery;
begin
  dsMail := CreateDS('SELECT * FROM RETURNMAIL WHERE 1=2');
  try
    dsMail.Open;
    dsMail.Append;
    dsMail.FieldByName('CONTRID').AsInteger := ContrID;
    dsMail.FieldByName('SIGN').AsString := frmMain.sign;
    if checkMailBooking.Checked then
      dsMail.FieldByName('MAILADRESS').AsString := edMailAdress.Text
    else
      dsMail.FieldByName('MAILADRESS').AsString := edMailAdress.Hint;
    dsMail.FieldByName('XMLDATA').AsString := PackData;
    dsMail.FieldByName('MESSAGE').AsString := StringReplace(StringReplace( ComboBox1.Text,' ','',[rfReplaceAll]),'ö','o',[rfReplaceAll]);
    dsMail.FieldByName('REGTIDPUNKT').AsDateTime := Now;
    dsMail.FieldByName('ANDRATTIDPUNKT').AsDateTime := Now;
    dsMail.FieldByName('ANDRATAV').AsString := frmMain.sign;
    dsMail.Post;
...
function CreateXMLTag(TagName, Value: string): string;
begin
  Result := '';
  if trim(Value) <> '' then
  begin
    Result := '<' + TagName + '>' + trim(Value) + '</' + TagName + '>'
  end;
end;

function TFrmKontrakt.PackData: string;
begin
  Result := '<KontraktData>';
  if checkMailBooking.Checked then
    Result := Result + CreateXMLTag('MailAdress', edMailAdress.Text)
  else
    Result := Result + CreateXMLTag('MailAdress', edMailAdress.Hint);

  Result := Result + CreateXMLTag('CustId', edtHpnum.Text);
  Result := Result + CreateXMLTag('CustNamn', edtHNamn.Text);
  Result := Result + CreateXMLTag('DriveId', edtFpnum.text);
  Result := Result + CreateXMLTag('DriverNamn', edtFNamn.text);
  Result := Result + CreateXMLTag('referens', edtRef.text);



  with ObjArr[1] do
  begin
    Result := Result + CreateXMLTag('obtypid', GetObjTypID(FcbObjId, cbObjTyp.items));
    Result := Result + CreateXMLTag('frm_time', DateTimeToStr(FDTVFrom));
    Result := Result + CreateXMLTag('to_time', DateTimeToStr(FDTVTO));
    Result := Result + CreateXMLTag('pclass', FedtPKlass);
    Result := Result + CreateXMLTag('ptype', FcbPType);
  end;

  Result := Result + '</KontraktData>';
