...
uses Login, Greg, Data2, Datamodule, Main, DateUtil; //!Kalender,
...
begin
  if (Sender as TEdit).Text < '!' then
    (Sender as TEdit).Text := VisibleDateToStr(now);
end;
...
begin
  try
    with (Sender as TEdit) do
      Text := Visibledatetostr(Visiblestrtodate(text));
  except
...
//!    Get_Date(Sender, 'Kostnadsdatum');
    if Key = VK_DOWN then // Öka tid
      (Sender as TEdit).Text := VisibleDatetostr(VisibleStrToDate((Sender as TEdit).Text) + 1);
