...
{**********************************************************************}
{}
{ $Log:  14874: Search.pas
{
{   Rev 1.6    2006-02-18 13:33:46  pb64
}
{
{   Rev 1.5    2005-07-04 09:40:44  pb64
{ Implementerat CarLegal + sparar uppgifter om momsfördelning.
}
{
{   Rev 1.4    2005-02-14 16:09:32  pb64
{ Infört debuginfo
}
{
{   Rev 1.3    2005-02-08 23:07:02  pb64    Version: 2005.01
{ Fixat Bug i sökhanteringen.
}
{
{   Rev 1.2    2005-02-08 14:59:58  pb64
{ Fixat så att man kan flytta fälten i sökgriden.
{ Använder fältnamn istället för ordningsnummer (som ändras)
}
{
{   Rev 1.1    2005-02-07 11:17:34  pb64
{ Lagt till Objekt typ i sök bild.
{ Ändrat SQL sats så att bokningar utan objekt eller kund visas upp i sök
{ bilden.
}
{
...
uses
  ...
  DBTables, ADODB, ActiveX, filectrl, Variants, JPeg;
...
type
  ...
  TfrmSearch = class(TForm)
    ...
    function SaveContrBase(ContractNo, CustId: Integer; const Sign,
      ...
      PayDays: Integer; Delfakt: Boolean; StartDatum, SlutDatum, Belopp: string; VatCostType: integer;
      Fakt2Datum, AckBelopp: string; MailBooking: Boolean; MailAdress: string): Byte;
    ...
    procedure SaveContrNot(ContrId: Integer; const CNot1, CNot2, INot1,
      INot2: WideString);
    procedure SaveContrImage(ContrId: Integer; const Image: TImage);
    procedure GetContrImage(ContrId: Integer; var res: TImage);
...
const
  ...
  SQLKontrakt =
    'SELECT Contr_objt.Status, Contr_base.ContrId , Contr_objt.ObTypId as Grupp, Contr_objt.OId AS RegNr, convert(varchar(16),Contr_objt.Frm_Time,20) AS Från, ' +
    'convert(varchar(16),Contr_objt.To_Time,20) AS Till, Customer.Name AS Namn, Contr_base.Referens AS Referens ' +
...
//    'WHERE  (Contr_base.ContrId = Contr_objt.ContrId) AND (Contr_base.CustID = Customer.Cust_Id) AND ((Contr_base.Status < 7) OR (Contr_base.Status = 8)) ' +
  'WHERE ((Contr_base.Status < 7) OR (Contr_base.Status = 8)) ' +
  ...
  SQLBokning =
    'SELECT Contr_objt.ObTypId as Grupp,Contr_objt.OId AS RegNr, convert(varchar(16),Contr_objt.Frm_Time,20) AS Från, ' +
    'convert(varchar(16),Contr_objt.To_Time,20) AS Till, Customer.Name AS Namn, Contr_base.Referens AS Referens, Contr_objt.Status, Contr_base.ContrId  ' +
    'FROM ([Contr_Base] AS Contr_base LEFT JOIN Customer ON Contr_base.CustID = Customer.Cust_Id) INNER JOIN [Contr_ObjT] AS Contr_objt ON Contr_base.ContrId = Contr_objt.ContrId ' +
  ...
  SQLBokningHelp =
    'SELECT Contr_objt.ObTypId as Grupp,Contr_objt.OId AS RegNr, convert(varchar(16),Contr_objt.Frm_Time,20) AS Från, ' +
    'convert(varchar(16),Contr_objt.To_Time,20) AS Till, Customer.Name AS Namn, Contr_base.Referens AS Referens, Contr_Not.CNot1 As [Notering], Contr_objt.Status, Contr_base.ContrId ' +
    'FROM ([Contr_Base] AS Contr_base LEFT JOIN Customer ON Contr_base.CustID = Customer.Cust_Id) INNER JOIN [Contr_ObjT] AS Contr_objt ON Contr_base.ContrId = Contr_objt.ContrId ' +
    'LEFT JOIN Contr_Not ON Contr_base.ContrId=Contr_Not.ContrId ' +
    'WHERE (((Contr_base.Status)<3)) ' +
    'ORDER BY Contr_objt.To_Time, Contr_objt.OId; ';

  SQLBokningHelp2 =
    'SELECT Contr_objt.ObTypId as Grupp,Contr_objt.OId AS RegNr, convert(varchar(16),Contr_objt.Frm_Time,20) AS Från, ' +
    'convert(varchar(16),Contr_objt.To_Time,20) AS Till, Customer.Name AS Namn, Contr_base.Referens AS Referens, Contr_Not.CNot1 As [Notering], Contr_objt.Status, Contr_base.ContrId, Station.Name as Station,  Objects.ClearForRent as Uthyrningsbar ' +
    'FROM ([Contr_Base] AS Contr_base LEFT JOIN Customer ON Contr_base.CustID = Customer.Cust_Id) INNER JOIN [Contr_ObjT] AS Contr_objt ON Contr_base.ContrId = Contr_objt.ContrId ' +
    'LEFT JOIN Contr_Not ON Contr_base.ContrId=Contr_Not.ContrId ' +
    'LEFT JOIN Objects ON Contr_objt.Oid=Objects.Reg_no ' +
    'LEFT JOIN Station ON Objects.Station=Station.StationId ' +
    'WHERE (((Contr_base.Status)<3)) ' +
    'ORDER BY Contr_objt.To_Time, Contr_objt.OId; ';
...
//!    'ORDER BY Contr_objt.To_Time DESC, Contr_objt.OId';
  ...
  'SELECT Contr_ObjT.Status, Contr_Base.ContrId, Contr_objt.ObTypId as Grupp, Contr_ObjT.OId AS RegNr, Convert(varchar(16),Contr_ObjT.Frm_Time,20) AS Från, ' +
    ' Convert(varchar(16),Contr_ObjT.To_Time,20) AS Till, Customer.Name AS [Kund/betalare], ' +
...
uses main, Greg, Datamodule, Data2, Kontrakt, DataSession, eqprn, DateUtil;
{$R *.DFM}
...
begin
  DebugInfo('+++ TfrmSearch.DBGrid1DblClick +++');
  Param1 := DBGrid1.DataSource.DataSet.FieldByName(ReturnField).AsString;
  modalresult := mrOK;
  DebugInfo('--- TfrmSearch.DBGrid1DblClick ---');
end;
...
begin
  DebugInfo('+++ TfrmSearch.FormActivate +++');
  ssearchcds.filtered := False;
  SSearchCDS.Filter := '';
  ...
  if caption = 'Sökresultat Pers/Org Nr' then SetGridKundId;
...
//  tempdays := 0;
  ...
  begin
    Filter := 'PKlass = ''' + string(PClass) + ''' AND PTYP = ''' + string(PType) + ''' AND FDat<=''' + internaldatetostr(From) + ''' AND TDat >=''' + internalDatetostr(From) + '''';
    Filtered := true;
...
begin
  ...
  begin
    ...
    try
      if MailBooking then
        FieldByName('MAILBOOKING').AsInteger := 1
      else
        FieldByName('MAILBOOKING').AsInteger := 0;
      FieldByName('MAILADRESS').AsString := MailAdress;
    except
    end;

    if VatCostType = 2 then
      FieldByName('VATCOSTTYPE').AsInteger := 2
    else
      FieldByName('VATCOSTTYPE').AsInteger := 1;
    ...
    FieldByName('DF_FAKT2DATUM').AsString := Fakt2Datum;
    try
      FieldByName('DF_ACKBELOPP').AsInteger := StrToInt(AckBelopp);
    except
      FieldByName('DF_ACKBELOPP').AsInteger := 0;
    end;
    Post;
...
begin
  ...
  while (dbgrid1.Columns.Items[i].Title.Caption <> ComboBox1.Text) and (i > 0) do
  begin
    dec(i);
  end;
  if i < 0 then i := 0;
...
begin
  eqprn.PrintKvitto(DBGrid1.DataSource.DataSet.FieldByName('ContrID').AsInteger, DBGrid1.DataSource.DataSet.FieldByName('SubID').AsInteger, 0, 1, true);
end;
...
procedure TfrmSearch.SaveContrImage(ContrId: Integer; const Image: TImage);
var
  str: TMemoryStream;
begin
//  Image.Picture.SaveToFile('bilder\' + IntToStr(ContrId) + '.jpg');
  ...
  try
    ...
    Dmod.Q3.FieldByName('Contrid').AsInteger := ContrId;

    Image.Picture.Graphic.SaveToStream(Str);

    TBlobField(Dmod.Q3.FieldByName('CARLEGAL')).LoadFromStream(Str);
...
procedure TfrmSearch.GetContrImage(ContrId: Integer; var res: TImage);
var
  str: TMemoryStream;
  jpg: TJPEGImage;
begin
  try
    ...
    dmod.Q3.SQL.Text := 'select * from Contr_Not where Contrid=' + dbdel + inttostr(ContrID) + dbdel;
    dmod.Q3.Active := True;
    if not dmod.Q3.Eof then
    begin
      jpg := TJPEGImage.Create;
      Str := TMemoryStream.Create;
      TBlobField(Dmod.Q3.FieldByName('CARLEGAL')).SaveToStream(Str);
      Str.Position := 0;
      if Str.Size > 2 then
      begin
        jpg.LoadFromStream(str);
        res.Picture.Assign(jpg);
      end
      else
        Res.Picture.Assign(nil);
      str.free;
      jpg.Free;
      Dmod.q3.Active := False;
    end
    else
      Res.Picture.Bitmap.FreeImage;
  except
  end;

{
  if FileExists('bilder\'+IntToStr(ContrId)+'.jpg') then
  Res.Picture.LoadFromFile('bilder\'+IntToStr(ContrId)+'.jpg')
  else
  Res.Picture.Bitmap.FreeImage;
}
