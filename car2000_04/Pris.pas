...
{ $Log:  14870: Pris.pas
{
{   Rev 1.4    2006-02-18 13:33:46  pb64
}
{
{   Rev 1.3    2005-02-07 11:20:44  pb64
{ Rutiner för att hantera den nya licensiering som sparas i databasen.
}
{
{   Rev 1.2    2004-11-12 14:25:10  pb64
}
{
{   Rev 1.1    2004-10-26 15:47:50  pb64
}
{
{   Rev 1.0    2004-08-18 11:01:02  pb64
...
function DelFakturera(Subid: Integer): Integer;
function DepositionFakturera(Subid: Integer): Integer;
procedure CheckHowToinvoice(InData: TADOQuery; StartDatum: TDateTime; var NoOfMonth: Integer; var Price: Real; Uppdatera: boolean = True);


function DoesEnummerExist(Enr: Integer): Boolean;
function IsContrIdFakturerad(Cid: Integer): Boolean;
...
procedure SetValueToInit(prog, sign, code, CodeValue: string);
...
var
  ...
  Value : Integer;
begin
  ds := CreateDS('SELECT E_FAKTNR FROM FAKTHEAD WHERE STATUS=0 AND KOREF=' + IntToStr(Cid));
  try
    ds.Open;
    Result := ds.Fields[0].AsInteger > 0;
    if Result then
      Value := ds.Fields[0].AsInteger;
  except
    Result := False;
  end;
  FreeDS(ds);
  if Result then
    Result := DoesEnummerExist(Value);
...
var
  ...
  i: Integer;
begin
  Q := CreateDS(format('UPDATE INITIERING SET CODEVALUE=''%s''  WHERE PROG=''%s'' AND SIGN=''%s'' AND CODE=''%s''', [CodeValue, Prog, Sign, Code]));
  try
    i := Q.ExecSQL;
    if i = 0 then
    begin
      SetDS(Q, format('INSERT INTO INITIERING (PROG,SIGN,CODE,CODEVALUE) VALUES (''%s'',''%s'',''%s'',''%s'')', [Prog, Sign, Code, CodeValue]));
      Q.ExecSQL;
    end;
  except
  end;
...
procedure SkapaLoggnr(ds: TADOQuery; var LoggNr: integer; ENummer: Integer);
begin
  if LoggNr < 0 then
    Loggnr := GetNextCounter('loggnr');

  SetDS(ds, Format('SELECT * FROM LOGGTABELL WHERE NRTYP=1 AND NUMMER=%d AND LOGGNR=%d',[ENummer,Loggnr]));
  ds.Open;
  if ds.IsEmpty then
    ds.Append
  else
    ds.Edit;
  ds.FieldByName('loggnr').AsInteger := Loggnr;
  ds.FieldByName('nrtyp').AsInteger := 1;
  ds.FieldByName('nummer').AsInteger := ENummer;
  ds.FieldByName('bokf_dag').AsDateTime := Trunc(now);
  ds.Post;
  ds.Close;
end;
...
begin
  ...
  inData := CreateDS(
    ...
    'contr_objt.Frm_Time, contr_objt.To_Time, contr_objt.OId, contr_objt.ObTypId,contr_objt.KM_In, contr_objt.KM_Out,objects.Model,objtype.Type, contr_subnot.faktnot1, contr_subnot.faktnot2, contr_sub.Terms_Pay, betalare.Lang ' +
  ...
  begin
    ...
    begin
      SetDS(tmpData, 'SELECT * FROM LOGGTABELL WHERE NUMMER=' + IntToStr(UtData.FieldByName('E_FAKTNR').AsInteger));
      tmpData.Open;
      if tmpData.IsEmpty then
      begin
    ...
    if UtData.FieldByName('FORFDAT').AsDateTime < 1.0 then
      UtData.FieldByName('FORFDAT').AsDateTime := UtData.FieldByName('FAKTDAT').AsDateTime + InData.FieldByName('Terms_Pay').AsInteger;
    ...
    UtData.FieldByName('STATUS').AsInteger := STAT_FAKTURA;
...
begin
  ...
  while (StartDatum + 30 < SlutDatum) do
  begin
    inc(NoOfMonth);
  ...
  begin
    InData.Edit;
    inData.FieldByName('DF_FAKT2DATUM').AsDateTime := trunc(StartDatum);
    inData.FieldByName('DF_ACKBELOPP').AsFloat := inData.FieldByName('DF_ACKBELOPP').AsFloat + Price;
    InData.Post;
  end;
end;
...
begin
  ...
  begin
    ...
    if (Paystr = 'K') or (PayStr = 'O') then PayStr := 'F';


    UtData.FieldByName('FAKTNR').AsInteger := NextFaktnr;
    Result := GetEnr(PayStr);
    UtData.FieldByName('E_FAKTNR').AsString := IntToStr(Result);
    UtData.FieldByName('PAYMENT').AsString := PayStr;
    UtData.FieldByName('RUBRIK').AsString := 'FAKTURA';
    ...
    UtData.FieldByName('SPRULE_KM').AsInteger := InData.FieldByName('SPRULE_KM').AsInteger;
    UtData.FieldByName('SPRULE_VAT').AsInteger := InData.FieldByName('SPRULE_VAT').AsInteger;
    UtData.FieldByName('STATUS').AsInteger := STAT_DELFAKTURA; // DELfAKTURA
    ...
    begin
      ...
      tmpData.FieldByName('TEXT').AsString := 'Delfaktura från '+VisibleDateToStr(inData.FieldByName('DF_FAKT2DATUM').AsDateTime-(NoOfMonth*30))+' till ' + inData.FieldByName('DF_FAKT2DATUM').AsString;
      tmpData.FieldByName('ANTAL').AsFloat := 1;
      ...
      tmpData.FieldByName('ANDRATTIDPUNKT').AsDateTime := Now;
      tmpData.FieldByName('ANDRATAV').AsString := 'IMP';
      VVarde := VVarde + Round(Price * 100) / 100.00;
      tmpData.Post;
    end;

    UtData.FieldByName('VARUVARDE').AsFloat := VVarde;
    UtData.FieldByName('MOMSSUM').AsFloat := Round(VVarde * 0.25 * 100) / 100.0;
    UtData.FieldByName('MOMSSUM_SEK').AsFloat := Round(VVarde * 0.25 * 100) / 100.0;
    UtData.FieldByName('FAKTSUM').AsFloat := Round(UtData.FieldByName('VARUVARDE').AsFloat + UtData.FieldByName('MOMSSUM').AsFloat);
    UtData.FieldByName('AVRUNDNING').AsFloat := UtData.FieldByName('FAKTSUM').AsFloat - (UtData.FieldByName('VARUVARDE').AsFloat + UtData.FieldByName('MOMSSUM').AsFloat);
    UtData.FieldByName('AVRUNDNING_SEK').AsFloat := UtData.FieldByName('AVRUNDNING').AsFloat;
    UtData.FieldByName('FAKTSUM_SEK').AsFloat := UtData.FieldByName('FAKTSUM').AsFloat;
  ...
  KonteraData(NextFaktnr);
  try
    SkapaLoggnr(InData, Loggnr, Result);
  except
  end;
  try
    FreeDS(UtData);
...
begin
  ...
  inData := CreateDS(
    ...
    'contr_sub.ContrId, contr_sub.SpRule_Rent, contr_sub.SpRule_KM, contr_sub.SpRule_Vat, betalare.Cust_Id, betalare.Name as BetName, betalare.Co_Adr, ' +
    'betalare.Adress,betalare.Postal_Name,betalare.Country, betalare.Org_No,signr.NAMN,customer.Name As CustName,driver.Name As DriverName,' +
    'contr_base.Referens,contr_base.Dep_Amount, contr_not.Cnot1, contr_not.Cnot2 ,Contr_base.Dep_Amount AS DSUM, 0 AS DMOMS, Contr_base.Dep_Amount AS DTOTAL,' +
    'contr_objt.Frm_Time, contr_objt.To_Time, contr_objt.OId, contr_objt.ObTypId,contr_objt.KM_In, contr_objt.KM_Out,objects.Model,objtype.Type, contr_subnot.faktnot1, contr_subnot.faktnot2 ' +
    'FROM Contr_Sub contr_sub LEFT OUTER JOIN Customer betalare ON contr_sub.SubCustId = betalare.Cust_Id INNER JOIN ' +
    'Contr_Base contr_base ON contr_base.ContrId = contr_sub.ContrId INNER JOIN Signr signr ON contr_base.Sign = signr.SIGN INNER JOIN ' +
    'Customer customer ON customer.Cust_Id = contr_base.CustID LEFT OUTER JOIN Customer driver ON contr_base.DriveId = driver.Cust_Id LEFT OUTER JOIN ' +
  ...
  begin
    ...
    SetDS(UtData, 'SELECT * FROM FAKTHEAD WHERE KOREF=' + InData.FieldByName('ContrId').AsString + ' AND KUNDNR=' + InData.FieldByName('Cust_Id').AsString + ' AND STATUS=' + IntToStr(STAT_DEPOSITION) + ' ORDER BY FAKTNR DESC');
    UtData.Open;
    ...
    begin
      SetDS(tmpData, 'SELECT * FROM LOGGTABELL WHERE NUMMER=' + IntToStr(UtData.FieldByName('E_FAKTNR').AsInteger));
      tmpData.Open;
      if tmpData.FieldByName('LOGGNR').AsInteger = 0 then
      begin
        NextFaktNr := UtData.FieldByName('FAKTNR').AsInteger;
        DmSession.ADOConnection1.Execute('DELETE FAKTRAD WHERE FAKTNR=' + IntToStr(NextFaktnr));
        DmSession.ADOConnection1.Execute('DELETE KNTERRAD WHERE NUMMER=' + IntToStr(NextFaktnr));
      ...
      begin
        Fakturerad_Summa := SumFaktFromKoref(InData.FieldByName('ContrId').AsInteger, STAT_DEPOSITION);
        UtData.Append;
      end;
      tmpData.Close;
    end
    ...
    PayStr := InData.FieldByName('PAYMENT').AsString;
    if (Paystr = 'F') then PayStr := 'K';

    UtData.FieldByName('FAKTNR').AsInteger := NextFaktnr;
    if utData.FieldByName('E_FAKTNR').IsNull then
    begin
      Result := GetEnr(PayStr)
    end
    else
    begin
      if (UtData.FieldByName('E_FAKTNR').AsInteger = 0) then
        Result := GetEnr(PayStr)
      else
        Result := UtData.FieldByName('E_FAKTNR').AsInteger;
    end;
    UtData.FieldByName('E_FAKTNR').AsString := IntToStr(Result);
    UtData.FieldByName('PAYMENT').AsString := PayStr;
    UtData.FieldByName('RUBRIK').AsString := 'FAKTURA';
    ...
    UtData.FieldByName('AVRUNDNING').AsFloat := 0;
    UtData.FieldByName('RABATT').AsFloat := 0;
    UtData.FieldByName('MOMSSUM').AsFloat := InData.FieldByName('DMOMS').AsFloat - Fakturerad_Summa;
    UtData.FieldByName('FAKTSUM').AsFloat := InData.FieldByName('DTOTAL').AsFloat - Fakturerad_Summa;
    UtData.FieldByName('BOKADKURS').AsFloat := 1;
    UtData.FieldByName('VALUTAKOD').AsString := 'SEK';
    UtData.FieldByName('AVRUNDNING_SEK').AsFloat := 0;
    UtData.FieldByName('RABATT_SEK').AsFloat := 0;
    UtData.FieldByName('MOMSSUM_SEK').AsFloat := InData.FieldByName('DMOMS').AsFloat - Fakturerad_Summa;
    UtData.FieldByName('FAKTSUM_SEK').AsFloat := InData.FieldByName('DTOTAL').AsFloat - Fakturerad_Summa;
    UtData.FieldByName('FRM_TIME').AsDateTime := InData.FieldByName('FRM_TIME').AsDateTime;
    UtData.FieldByName('TO_TIME').AsDateTime := InData.FieldByName('TO_TIME').AsDateTime;
    UtData.FieldByName('OID').AsString := InData.FieldByName('OID').AsString;
    UtData.FieldByName('OBTYPID').AsString := InData.FieldByName('OBTYPID').AsString;
    ...
    UtData.FieldByName('SPRULE_KM').AsInteger := InData.FieldByName('SPRULE_KM').AsInteger;
    UtData.FieldByName('SPRULE_VAT').AsInteger := InData.FieldByName('SPRULE_VAT').AsInteger;
    UtData.FieldByName('STATUS').AsInteger := STAT_DEPOSITION; // DELfAKTURA
    UtData.FieldByName('LANGUAGE').AsString := 'SE';
    UtData.FieldByName('REGTIDPUNKT').AsDateTime := Now;
    UtData.FieldByName('ANDRATTIDPUNKT').AsDateTime := Now;
    UtData.FieldByName('ANDRATAV').AsString := 'IMP';
    ...
    if not InData.IsEmpty then
    begin
      Price := inData.FieldByName('Dep_Amount').AsFloat - Fakturerad_Summa;
      tmpData.Append;
      tmpData.FieldByName('FAKTNR').AsInteger := NextFaktnr;
      tmpData.FieldByName('RAD').AsInteger := 1000;
      tmpData.FieldByName('TEXT').AsString := 'Deposition för kontrakt ' + inData.FieldByName('ContrID').AsString;
      tmpData.FieldByName('ANTAL').AsFloat := 1;
      tmpData.FieldByName('A_PRIS').AsFloat := Round(Price * 100) / 100.00;
      tmpData.FieldByName('A_PRIS_SEK').AsFloat := Round(Price * 100) / 100.00;
      tmpData.FieldByName('RABATT').AsFloat := 0;
