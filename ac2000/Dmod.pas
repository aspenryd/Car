...
{   Rev 1.3    2005-04-28 15:57:42  pb64
{ Fixat så att kund och object markerar spara knappen vid nytt.
}
{
{   Rev 1.2    2005-02-07 13:15:44  pb64
}
{
...
type
  TDmod2 = class(TDataModule)
    ...
    FaktHeadT: TADOTable;
    FaktRadT: TADOTable;
    EDIBaseS: TDataSource;
    EDIBaseT: TADOTable;
    EDICustS: TDataSource;
    EDICustT: TADOTable;
    ...
    procedure ObjectSStateChange(Sender: TObject);
    procedure CustomerSStateChange(Sender: TObject);
    procedure StationSStateChange(Sender: TObject);
    procedure EDIBaseSStateChange(Sender: TObject);
    procedure EDICustSStateChange(Sender: TObject);
  private
...
procedure TDmod2.ObjectSStateChange(Sender: TObject);
begin
  if ObjectS.state in [dsedit, dsinsert] then
  begin
    frmMain.BtnCan.Enabled := True;
    frmMain.btnsave.Enabled := True;
  end
...
procedure TDmod2.CustomerSStateChange(Sender: TObject);
begin
...
procedure TDmod2.StationSStateChange(Sender: TObject);
begin
  if StationS.state in [dsedit, dsinsert] then
  begin
    frmMain.BtnCan.Enabled := True;
    frmMain.btnsave.Enabled := True;
  end
...
procedure TDmod2.EDIBaseSStateChange(Sender: TObject);
begin
  if EdiBaseS.state in [dsedit, dsinsert] then
  begin
    frmMain.BtnCan.Enabled := True;
    frmMain.btnsave.Enabled := True;
  end
...
procedure TDmod2.EDICustSStateChange(Sender: TObject);
begin
  if EdiCustS.state in [dsedit, dsinsert] then
  begin
    frmMain.BtnCan.Enabled := True;
    frmMain.btnsave.Enabled := True;
  end
