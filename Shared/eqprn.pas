...
{ $Log:  13182: eqprn.pas 
{
{   Rev 1.6    2006-02-18 13:33:20  pb64
}
{
{   Rev 1.5    2005-04-28 16:02:12  pb64
{ Lagt till hantering för att skriva ut fakturor utan att den skall finnas i
{ contr_sub. t.ex. kredit och delfaktura.
}
{
{   Rev 1.4    2005-02-07 11:24:10  pb64
}
{
{   Rev 1.3    2004-12-07 16:37:50  pb64
...
uses
  SysUtils, WinTypes, WinProcs, Dialogs, Controls, ADOdb, EQPrinterdata,
...
begin
  ...
  ADOConn.ConnectionString := Main.DbWay;
...

{
  ...
  with ADOQ1.SQL do begin
   ...
   begin
     clear;
     Add('SELECT e_faktnr as enummer from fakthead where e_faktnr='+IntToStr(ENummer));
   end
   else begin
     if MainSub <> 0 then
...
begin
    ...
    with Q.SQL do begin
      Clear;
      Add('SELECT Ini AS Ini FROM Reports WHERE Program='''+REPORTID+''' AND CallName = '''+ident+'''');
    end;
    ...
    try
      ...
      try
        Result := TStringStream.Create(Q.FieldByName('Ini').AsString);
      except
      end;
    end;
    Q.Close;
