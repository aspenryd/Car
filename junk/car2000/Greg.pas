...
begin
  ...
  begin
    ...
    try
      sl.LoadFromFile('cardirect.txt');
      if sl.Count>0 then
      begin
        tmp := StrToFloat(sl[1]);
        if tmp+(15.0/86400)>Now then
        begin
          Regnr := ObjectsS.DataSet.FieldByName('Reg_No').AsString;
          try
            GenericExecSQL('UPDATE OBJECTS SET CARDIRECT='''' WHERE CARDIRECT='''+Sl[0]+''' AND REG_NO<>'''+REGNR+'''');
          except
          end;
          if ObjectsS.state in [dsedit, dsinsert] then
          begin
            ObjectsS.DataSet.Edit;
            ObjectsS.DataSet.FieldByName('CARDIRECT').AsString := sl[0];
          end
          else
          begin
            GenericExecSQL('UPDATE OBJECTS SET CARDIRECT='''+Sl[0]+''' WHERE REG_NO='''+Regnr+'''');
