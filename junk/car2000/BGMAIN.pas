...
var
  ...
  ds: TADOQuery;
begin
  ds := CreateDS('SELECT Objects.Reg_No, Objects.ObjNum, Objects.Model, Objects.Type, Objects.KM_N, Objects.DType, Objects.Color, Objects.Accesories, Objects.VStat, Objects.Note FROM Objects where Objects.VStat='+edtVstat.Text+' Order by Objects.Reg_No');
  try
    ds.Open;
    ...
    begin
      ...
      begin
        ...
        Hinttext := Hinttext + ' Mätarställning : ' + ds.FieldByName('KM_N').AsString;
...
begin
  ds := CreateDS('SELECT Contr_Base.ContrId, Contr_Base.CustID, Contr_Base.Sign, Customer.Name, Contr_ObjT.OId, Contr_ObjT.Frm_Time, Contr_ObjT.To_Time, Contr_Base.Status, Contr_Base.Referens' +
    ' FROM Contr_Base LEFT JOIN Contr_ObjT ON Contr_Base.ContrId = Contr_ObjT.ContrId LEFT JOIN Customer ON Contr_Base.CustID = Customer.Cust_Id WHERE Contr_ObjT.To_Time>''' + datetostr(datetimepicker1.Date - Bak_Datum) + ''' ');
  try
    ds.Open;
...
begin
 ...
 // De följande raderna kan jag inte förstå varför dem finns.
 // men jag vågar inte ta bort dem. /PB
 // --- Start ---
  frmmain.iniT.Open;
