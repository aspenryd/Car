...
uses
  ...
  Grids, comp2000, ActnList, tmpData, eqprn, divutils;
...
const
  stdDays = 10; //! Kanske skall hämtas från Param iStället
...
var
  ...
  pterms, i: integer;
begin
  frmSearch.GSQLString := 'Select Customer.name, Customer.Adress, Customer.Postal_name, Customer.Tel_nr_1, Customer.Org_no, Customer.KTyp, Customer.KontoNr, Customer.PTyp, Customer.Payment, Customer.Terms_pay, Customer.KExp, Customer.Driver from Customer Where Cust_id = ' + cuid;
  if frmSearch.GetSearchResult then
    with frmSearch.GSearchCDS do
    begin
...
begin
  frmSearch.GSQLString := 'Select Customer.name, Customer.Adress, Customer.Postal_name, Customer.Tel_nr_1, Customer.Org_no from Customer Where Cust_id = ' + custid;
  if frmSearch.GetSearchResult then
    with frmSearch.GSearchCDS do
    begin
...
//!Hit
            ...
            if currentview in [Booking, Contract] then
//!benny Lägger till 0820
            begin
//!Hit
              edtKM_BER.text := fieldbyname('KM_Ber').AsString; //!lagt till ;
//!Benny Lägger till 0820
              if frmgreg.ObjectsT.Locate('Reg_No', Bilnr, [locaseinsensitive]) then
              begin
                if (CurrentView = Contract) and ((edtKM_ut.Text = '0') or (edtKM_ut.Text = '')) then
                  edtKM_ut.Text := frmgreg.objectsT.fieldbyname('KM_N').asstring;
              end
              else
              begin
...
//!    PNumR:=PNumR+'-'+Copy(dmsession.SDbAnswerT.fieldbyname('KundNr').asString,7,4);
      ...
      begin
        ...
        frmgreg.CustomerT.fieldbyname('Name').AsString := ds.fieldbyname('ENamn_CBR').asstring + ', ' + ds.fieldbyname('FNamn_CBR').asstring;
