...
uses
  ...
  StdCtrls, Grids, DB, DBTables, DBClient, ExtCtrls, Variants; //!, BDEProv

const
  ActionStr: array[TReconcileAction] of string = ('Hoppa över', 'Avsluta', 'Foga samman',
    'Rätta', 'Avbryt', 'Uppdatera');
  UpdateKindStr: array[TUpdateKind] of string = ('Ändrad', 'Infogad',
    'Borttagen');
  SCaption = 'Uppdateringsfel - %s';
  SUnchanged = '<Oförändrad>';
  SBinary = '(Binär)';
  SFieldName = 'Fältnamn';
  SOriginal = 'Original värde';
  SConflict = 'Värde i konflikt';
  SValue = ' värde';
  SNoData = '<Inga poster>';
  SNew = 'Ny';
...
begin
  try
    if VarIsEmpty(V) then
      ...
      if DataType in BinaryDataTypes then
        Result := SBinary
      else
