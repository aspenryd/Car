...
begin
  ...
  dmod.Q3.SQL.Text := 'select * from Contr_Base where Contrid=' + dbdel + inttostr(ContractNo) + dbdel;
  dmod.Q3.Active := True;
...
//! Update
    ...
    dmod.Q4.SQL.Text := 'Update Contr_Base set Custid=' + DBDel + inttostr(CustId) + DBDel + ', Contr_Date=' + DBDel + DateTimeToStr(Now) + DBDel + ', Sign=' + DBDel + Sign + DBDel + ',Referens=' + DBDel + Referens + DBDel + ', Status=' + DBDel + inttostr(Status) + DBDel + ', Payment=' + DBDel + Payment + DBDel + ',[Pay-fact]=' + DBDel + Inttostr(PayDays) + DBDel + ' where  contrid=' + DBDel + inttostr(ContractNo) + DBDel + '';
    dmod.Q4.ExecSQL;
  end else
  begin
//! Insert
    dmod.Q4.Active := False;
    dmod.Q4.SQL.Text := 'Insert into Contr_Base(ContrId,Custid,Contr_Date, Sign,Referens,Status,Payment,[Pay-fact]) Values (' + dbdel + Inttostr(ContractNo) + dbdel + ',' + dbdel + inttostr(CustId) + dbdel + ',' + DBDel + DateTimeToStr(Now) + DBDel + ',' + dbdel + Sign + dbdel + ',' + dbdel + Referens + dbdel + ',' + dbdel + inttostr(Status) + dbdel + ',' + dbdel + Payment + dbdel + ',' + dbdel + Inttostr(PayDays) + dbdel + ')';
    dmod.Q4.ExecSQL;
    result := 1;
  end;
  dmod.Q3.Active := False;
